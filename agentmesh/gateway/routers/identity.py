...
class RegisterAgentRequest(BaseModel):
    ...
    Attributes:
        ...
        auth_token: Optional auth token (Feishu user_id / webhook secret).
        metadata: Optional JSON-serialisable extensions.
    """
    name: str
    auth_token: str = ""
...
def register_agent(body: RegisterAgentRequest):
    """Register a new agent and return its identity details.

    Generates an Ed25519 key pair, creates a DID, and persists
    the identity to the database.

    Args:
        body: Registration request with name and optional metadata.

    Returns:
        JSON response with agent_id, did, name, public_key, created_at.
    """
...
def get_agent(agent_id: str):
    ...
    Args:
        agent_id: UUID of the agent to find.
