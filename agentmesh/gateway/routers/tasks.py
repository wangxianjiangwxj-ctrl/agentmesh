...
from agentmesh.task_market_api import CreateTaskRequest, TaskStatus
...
class DeliverTaskBody(BaseModel):
    """Request body for delivering a completed task."""
    submission: str  # delivery URL
    executor_id: str


class VerifyTaskBody(BaseModel):
    """Request body for verifying a delivered task."""
    approved: bool = True
    publisher_id: str
...
async def list_tasks(
    ...
    """List tasks with optional status and publisher filters."""
    svc = get_task_market_service()
    task_status = None
    if status:
        try:
