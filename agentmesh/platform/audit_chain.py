...
class AuditChainService:
    ...
    def record(
        ...
        *,
        ...
        sender_sig, receiver_sig = self._make_signatures(actor_id, receiver_id, payload)
        prev_hash = self._get_chain_head(task_id)
        chain_hash = self._compute_chain_hash(entry_id, prev_hash, payload_digest)

        entry = self._build_entry(
            entry_id, task_id, action, actor_id,
            payload_digest, sender_sig, receiver_sig,
            prev_hash, chain_hash, extra,
        )
        self._persist_entry(entry, chain_hash, task_id)
    ...
    def _make_signatures(self, actor_id: str, receiver_id: Optional[str], payload: dict) -> tuple[str, Optional[str]]:
        """Create sender and optional receiver signatures.

        Signs the payload with the actor's private key.  If a receiver_id
        is provided, also creates a double-signature envelope.

        Args:
            actor_id: Sender agent identifier.
            receiver_id: Optional receiver agent identifier.
            payload: The payload dict to sign.

        Returns:
            A tuple of (sender_sig, optional receiver_sig).
    ...
    def _compute_chain_hash(entry_id: str, prev_hash: Optional[str], digest: str) -> str:
        """Compute the hash-chain hash for an audit entry.
    ...
    def _build_entry(
        ...
        """Build an AuditEntry dataclass instance.
        ...
        Args:
            ...
            receiver_sig: Optional receiver's signature.
            prev_hash: Previous chain hash.
            chain_hash: Current chain hash.
            extra: Optional extra metadata dict.

        Returns:
            An AuditEntry instance (created_at is set by DB).
        """
        return AuditEntry(
            id=entry_id,
            task_id=task_id,
    ...
    def _persist_entry(self, entry: AuditEntry, chain_hash: str, task_id: str) -> None:
        """Insert the audit entry and update the chain head in a transaction.

        Args:
            entry: The AuditEntry to persist.
            chain_hash: The chain hash to store in the head table.
    ...
    def _fetch_entry(self, entry_id: str) -> Optional[AuditEntry]:
        """Fetch an audit entry back from the database by ID.
        ...
        Returns:
            An AuditEntry with the DB-assigned timestamp, or None.
        """
        row = self.conn.execute(
